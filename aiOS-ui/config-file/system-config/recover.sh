...
if [ -z "$WEBUI_PID" ]; then
  ...
  if [ "$(id -u)" -eq 0 ]; then
    ...
    env HOME=/home/ai_user HERMES_WEBUI_HOST=0.0.0.0 HERMES_WEBUI_PORT=8501 HERMES_WEBUI_TRUST_FORWARDED_HOST=1 "$PYTHON_EXE" /aiOS-ui/features/hermes-webui/server.py >>/config-file/aiOS-ui.log 2>&1 &
  fi
  WEBUI_PID=$!
