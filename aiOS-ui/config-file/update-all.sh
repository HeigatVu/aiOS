...
if [ -f /.dockerenv ] || [ "${USER:-}" = "ai_user" ]; then
  ...
  HERMES_WEBUI_DIR="/aiOS-ui/features/hermes-webui"
  ...
  HERMES_WEBUI_DIR="${ROOT_DIR}/features/hermes-webui"
...
safe_git_pull() {
  ...
  echo "→ Pulling updates from ${remote} (${current_branch})..."
  # Try to pull active branch, then main, then master
  local pull_failed=0
  if ! (git pull "$remote" "$current_branch" 2>/dev/null || git pull "$remote" main 2>/dev/null || git pull "$remote" master); then
    pull_failed=1
  fi
