...
const LISTEN_PORT = 3113;
const TARGET_HOST = "127.0.0.1";
const TARGET_PORT = 3113;
...
// Rewrite Host header so the viewer's allowlist check passes.
function rewriteHostHeader(buf, lanIP, port) {
  const str = buf.toString("binary");
  const headerEnd = str.indexOf("\r\n\r\n");
  if (headerEnd === -1) return buf; // incomplete headers — pass through unchanged
...
server.listen(LISTEN_PORT, LISTEN_HOST, () => {
  console.log(`[viewer-proxy] ${LISTEN_HOST}:${LISTEN_PORT} -> ${TARGET_HOST}:${TARGET_PORT} (host-rewrite enabled)`);
});
