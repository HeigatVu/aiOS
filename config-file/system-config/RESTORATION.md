...
dashboard, dashboard-proxy, hermes gateway. It is the single
...
| Bug                                                                                                      | Fix                                                                                            |
| -------------------------------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------- |
| /proc scan used SIGTERM: process could ignore, port not released → EADDRINUSE                            | Changed to `kill -9` (SIGKILL) + port-release polling via `/proc/net/tcp` (up to 5s for proxy) |
